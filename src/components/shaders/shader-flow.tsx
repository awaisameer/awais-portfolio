...
export function ShaderFlow(props: ShaderFlowProps): ReactNode {
  ...
  useEffect(() => {
    ...
    // Some machines / browsers have WebGL disabled. Fail quietly instead of
    // throwing, which would tear down the whole React tree.
    let r: Renderer;
    try {
      r = new Renderer({
      ...
      console.warn("WebGL unavailable: skipping animated background.", error);
      return;
    }
    const gl = r.gl;
